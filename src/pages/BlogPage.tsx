...
export const BlogPage = () => {
  ...
  useEffect(() => {
    sanityClient
      .fetch<PostListItem[]>(postsListQuery)
      .then(setPosts)
        ...
        <div className="mx-auto w-full max-w-[1440px]">
          {error && (
            <div className="rounded border border-red-300 bg-red-50 p-5 text-sm text-red-800">
              Couldn’t load posts: {error}. If this is a CORS error, add{" "}
              <code className="text-red-900">{location.origin}</code> at
              sanity.io/manage → Blog project → API → CORS origins.
            </div>
          )}
