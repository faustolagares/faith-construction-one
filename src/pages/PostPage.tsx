...
export const PostPage = () => {
  ...
  useEffect(() => {
    ...
    sanityClient
      ...
      .then((p) => {
        ...
        setPost(p);
        document.title = p.seo?.metaTitle || `${p.title} — Faith Construction`;
        const categoryRefs =
          p.categories?.map((c) => c._id).filter(Boolean) || [];
        if (categoryRefs.length > 0) {
          sanityClient
            .fetch<PostListItem[]>(relatedPostsQuery, {
              slug,
              categoryRefs,
            })
            .then(setRelated)
    ...
    return () => {
      document.title = "Faith Construction";
    };
  }, [slug]);
  ...
  if (error) {
    return (
      <Shell>
        <div className="px-5 md:px-8 lg:px-10 py-24">
          <div className="mx-auto w-full max-w-[760px] rounded border border-red-300 bg-red-50 p-6 text-red-800 text-sm">
  ...
  if (post === null) {
    return (
      <Shell>
        <div className="px-5 md:px-8 lg:px-10 py-24 text-slate-500 text-sm">
          Loading…
        </div>
      </Shell>
    );
      ...
      <Shell>
        <div className="px-5 md:px-8 lg:px-10 py-24">
          <div className="mx-auto w-full max-w-[760px] text-center">
            <p className="text-red-600 text-[11px] font-bold tracking-[2.64px] uppercase mb-4">
              404
            </p>
            <h1 className="font-playfair_display text-4xl md:text-5xl tracking-[-0.8px] leading-tight mb-4 text-slate-900">
            ...
            <Link
              to="/blog"
              className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[1.76px] uppercase text-slate-900 hover:text-red-600"
            >
              <span className="h-px w-6 bg-red-600" /> Back to blog
            </Link>
          </div>
        </div>
      </Shell>
          ...
          <div className="mx-auto w-full max-w-[760px]">
            ...
            {post.author && (
              <motion.aside
                ...
                whileInView="visible"
                viewport={viewport}
                className="mt-16 md:mt-20 border-t border-slate-200 pt-10 md:pt-12"
              >
